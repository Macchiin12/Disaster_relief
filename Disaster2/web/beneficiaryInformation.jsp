...
<head>
    ...
    <title>Disaster Relief - Beneficiary Information</title>
    ...
    <script>
        function confirmDelete(beneficiaryId) {
            if (confirm("Are you sure you want to delete beneficiary ID: " + beneficiaryId + "?")) {
                window.location.href = "beneficiaryInformation.jsp?action=delete&id=" + beneficiaryId;
            }
        }
    </script>
</head>
            ...
            <div class="data-card">
                ...
                <%
                    ...
                    if ("success".equals(statusMessage)) {
                        out.println("<p style='color: green; font-weight: bold;'>Beneficiary registered successfully!</p>");
                    } else if ("updated".equals(statusMessage)) {
                        out.println("<p style='color: green; font-weight: bold;'>Beneficiary updated successfully!</p>");
                    } else if ("deleted".equals(statusMessage)) {
                        out.println("<p style='color: green; font-weight: bold;'>Beneficiary deleted successfully!</p>");
                    ...
                    try {
                        ...
                        if ("delete".equals(request.getParameter("action"))) {
                            ...
                            if (idToDelete != null && !idToDelete.isEmpty()) {
                                boolean deleted = beneficiaryDao.deleteBeneficiary(idToDelete);
                                if (deleted) {
                                    response.sendRedirect("beneficiaryInformation.jsp?status=deleted");
                                    return; // Important to prevent further processing after redirect
                                } else {
                                    response.sendRedirect("beneficiaryInformation.jsp?status=error");
                                    return;
                                }
                            }
                        }
                        ...
                        if (beneficiaries.isEmpty()) {
                            out.println("<p class='no-records'>No beneficiary records found.</p>");
                        } else {
                %>
                <table>
                    <thead>
                    ...
                    <tbody>
                        <%
                            // Enhanced for-loop is Java 5+, which is fine for your 1.5 target.
                            // If you were truly restricted to pre-Java 5, you'd need an Iterator.
                            for (BeneficiaryBean beneficiary : beneficiaries) {
                        %>
                        <tr>
                            <td><%= beneficiary.getBeneficiaryId() %></td>
                            <td><%= beneficiary.getBeneficiaryName() %></td>
                            <td><%= beneficiary.getIcNumber() %></td>
                            <td><%= beneficiary.getAddress() %></td>
                            <td><%= beneficiary.getContactNumber() %></td>
                            <td><%= beneficiary.getDateOfBirth() %></td>
                            <td class="actions">
                                <a href="editBeneficiaryInformation.jsp?id=<%= beneficiary.getBeneficiaryId() %>">Edit</a>
                                <a href="javascript:void(0);" class="delete-btn" onclick="confirmDelete('<%= beneficiary.getBeneficiaryId() %>');">Delete</a>
                            </td>
                        </tr>
                        <%
                            }
